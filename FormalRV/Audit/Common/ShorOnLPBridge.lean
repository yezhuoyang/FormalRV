/-
  FormalRV.Audit.Common.ShorOnLPBridge — connect the Shor-algorithm island to the LP-code island
...
theorem multiplier_gateset_bridges_to_LP
  ...
  ⟨FormalRV.Audit.Common.ShorPPMUnitaryReduction.ppm_clifford_run_eq_applyNat F g hICX f σ' hrun,
   (FormalRV.Audit.Common.TeleportCCXGrounded.teleportCCX_grounded_in_verified_clifford_T
      F a b c s t hac hbc h).1⟩

end FormalRV.Audit.Common.ShorOnLPBridge
