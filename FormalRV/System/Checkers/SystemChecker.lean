...
/-! ### Strengthening status: IMPLEMENTED.
    ...
    §6) requires every `PauliFrameUpdate cid` to begin after a matching
    `DecodeSyndrome cid` ends; `decoder_dependency_violator_now_rejected`
    there proves the strict bundle rejects this schedule. -/
...
/-! ### Strengthening status: OPEN.
    ...
    ids consumed by `gate_id` (a `routing_lane_fn : gate_id → List
    RoutingSiteId`), making `exclusivity_ok` a multi-resource check.
    Until then, schedules that pass `exclusivity_ok` are correct
    only on architectures with FULL connectivity (any two endpoint
    pairs can fire simultaneously).  Regression kept:
    `strict_still_accepts_routing_lane_violator`
    (Invariants/SystemInvariantStrengthening §11). -/
...
/-! ### Strengthening status: IMPLEMENTED.
    ...
    §17–§19) runs exactly this stateful lifecycle walk (fresh →
    in-use → dirty, consumed by Measure);
    `freshness_use_before_reset_now_rejected` and
    `freshness_reuse_without_reset_now_rejected` there prove the
    strict-with-freshness bundle rejects both schedules above. -/
...
/-! ### Strengthening status: OPEN.
    ...
    (`structure SiteId where val : Nat`, …) instead of an `abbrev`,
    forcing explicit conversions at the type level; less invasively,
    partition the `Nat` range by zone role and audit that no `Gate2q`
    endpoint lands in a factory-port-reserved subrange. -/
...
/-! ### Strengthening status: OPEN.
    ...
    request begins no earlier than `(i + 1) · distillation_us`.
    `window_throughput_ok` is a weaker AGGREGATE bound that accepts
    any distribution as long as no single window over-subscribes.
    Regression kept: `strict_still_accepts_magic_no_startup_prefix`
    (Invariants/SystemInvariantStrengthening §11). -/

/-! ## §9. `capacity_per_cycle_ok` is NOT redundant

    One might hope that under `capacity_in_arch_ok ∧
    exclusivity_ok`, `capacity_per_cycle_ok` is implied: under
    exclusivity, simultaneously-active SysCalls claim disjoint
    atoms, so the occurrence count in
    `((active_at t).flatMap syscall_acts_on)` would equal the
    distinct-atom count, which in-arch then bounds by each
    zone's capacity.

    That argument FAILS within a single SysCall: a degenerate
    `Gate2q q q` claims `[q, q]` — two occurrences of the same
    atom from ONE SysCall.  `exclusivity_ok` compares only
    DISTINCT overlapping SysCalls (pairs `i < j`), never a
    SysCall against itself, and `capacity_in_arch_ok` only
    checks zone membership.  So a zone of capacity 1 around `q`
    passes both yet fails `capacity_per_cycle_ok` (occurrence
    count 2 > 1).  The per-cycle conjunct is therefore the only
    one that catches within-SysCall atom aliasing; it stays in
    the bundle on its own merit, not as defence-in-depth. -/
...
/-! ## §11. Summary table (machine-readable)
    ...
    Counterexamples in this file (all ACCEPTED by the base
    bundle `all_invariants_with_factory_ports_ok`):

      §3   target_pauli ignored                   gap — open
      §4   decoder → PauliFrame ordering          gap — closed by
                                                  `feedback_after_decode_ok`
      §5   Gate2q routing/coupler                 gap — open
      §6   RequestFreshAncilla freshness          gap — closed by
                                                  `ancilla_freshness_ok`
      §7   SiteId type-conflation                 gap — open
      §8   window_throughput causal-prefix        gap — open
    ...
    Non-redundancy fact:
      ...
      §9   capacity_per_cycle_ok catches within-
           SysCall atom aliasing (Gate2q q q)
           that exclusivity ∧ in-arch cannot      OK
    ...
    The headline finding: **the base bundle is sound for the
    resource categories it tracks (atom-site exclusivity,
    ...
    The strict bundles in `Invariants/SystemInvariantStrengthening`
    close (ii) and (iv) and keep (i), (iii), (v) as named TODO
    regressions; the counterexamples in this file are the
    regression corpus those repairs must continue to reject. -/

end FormalRV.System.SystemChecker
