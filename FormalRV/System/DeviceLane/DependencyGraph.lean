...
/-! ## (3) No self-dependencies.
    ...
    Only the SELF-EDGE case of acyclicity is proven here: with a positive
    operation duration, a self-edge `i → i` would force `end_i ≤ begin_i`,
    contradicting `begin_i < end_i`.  Longer cycles would be excluded by the same
    strict-time-decrease argument, but that induction is NOT formalized — this
    theorem alone does not establish acyclicity of respected graphs. -/
...
/-! ## (6) Causal depth = the runtime floor.
    ...
    layers, p. 6, p. 24) — the floor the `System/Bounds/NaiveUpperBound.lean`
    sequential makespan rests on.  Transitivity along a 2-edge chain `i → j → k`: -/
...
/-! ## (7) A CONDITIONAL critical-path runtime LOWER BOUND.
    ...
    CAREFUL ABOUT WHAT THIS IS.  An *unconditional* lower
