...
/-! ## §3. `routingQubits` is a *reserved* cost (consistency of the cost primitive). -/

/-- The routing cost is realized in the footprint: the footprint has exactly
...
/-- A surface-code PPM's `routingQubits` is exactly the number of ancilla-path resources it
...
/-- **Conflict / latency / throughput are SHARED.**  Changing an operation's routing kind
