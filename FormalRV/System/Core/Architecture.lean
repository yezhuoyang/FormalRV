...
inductive SysCallKind
  ...
  /-- Request a fresh (reset) ancilla AT THE NAMED SITE.  The site is
      explicit — qubits are never fungible in an executable schedule, so
      "give me one ancilla from zone z" is not expressible; the scheduler
      must say WHICH ancilla qubit it resets/allocates.  The freshness
      checker validates the site's lifecycle (must not be Live) and which
      ancilla zone, if any, tracks it. -/
  | RequestFreshAncilla (site : Nat)
...
structure Architecture where
  ...
  /-- Time budget for one stabilizer-measurement cycle, in µs.
      Recorded as cited platform data; no checker in this file or
      elsewhere in the repo currently consumes it. -/
  t_stab_cycle_us  : Nat
  /-- Maximum allowed classical-decoder reaction time, in µs.
      Consumed by `latency_ok` (bounds `DecodeSyndrome` duration). -/
  t_react_us       : Nat
  /-- Coherence time, in µs.  Recorded as cited platform data; no
      checker consumes it — only a lookup smoke test at the end of
      this file reads it. -/
  t_coherence_us   : Nat
  deriving Repr
...
/-! ## Structural preconditions per SysCall

    Philosophy: *well-formedness first, resource verification
    next*.  This block encodes the STRUCTURAL preconditions every
    SysCall must satisfy to be a well-formed operation in the
    schedule's context.  No unitary semantics is checked anywhere
    in this block.
    ...
    A schedule that violates any of these is *structurally
    ill-formed* — it's not just inefficient on the chosen
    architecture; it doesn't compile to a valid quantum operation
    on ANY hardware.

    Resource verification is meaningful ONLY for schedules that
    pass these checks.  Otherwise we'd be counting resources for
...
/-- **Structural precondition check.**  Every SysCall has its
...
/-! ## Magic-state cost specification
    ...
    * `output_fidelity_x1e6` — output-state fidelity × 10⁶
    ...
    Magic states are the most expensive resource: even left as a
    black box, the factory's cost in qubits / time / failure rate /
    fidelity is recorded explicitly. -/
...
/-! ## LogicalLayout — synthesis between logical and physical layers
    ...
    This is the LOGICAL LAYOUT SYNTHESIS layer: the infrastructure
    is complete enough for users to specify every detail of
    logical-gate layout and factory scheduling.
