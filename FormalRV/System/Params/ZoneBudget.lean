...
/-- The FIRST zone's capacity in the built layout is exactly its user-set count.  (Only the head
...
/-- The total is monotone (non-strict, `≤`) in the MEMORY budget `N_m`.  Only this one zone is
...
end FormalRV.System.ZoneBudget
