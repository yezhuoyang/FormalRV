...
namespace FormalRV.System.ScheduleInv
...
/-! ## I5: syndrome-data streaming bandwidth (bit accounting)

    QEC produces a CONTINUOUS stream of syndrome data that must reach the
    decoder over a finite classical link.  Each `Measure` injects
    `bits_per_measure` bits into the stream (a hardware fact: 1 for hard
    readout; more for soft information or when one `Measure` op stands for
    a whole patch's stabilizer readout — e.g. a d=3 patch read out as one
    op contributes 8 stabilizers × 8-bit soft data = 64 bits).  The link
    contract is `max_bits` per `window_us` window — e.g. 4 KB/ms is
    `window_us = 1000`, `max_bits = 32768`.

    Decidable by the same window-anchoring argument as I4: the bit count
    of an arbitrary window is dominated by some window anchored at a
    measurement's begin time. -/

/-- Total syndrome bits injected by measurements BEGINNING in
    `[t0, t0 + window_us)`. -/
def syndrome_bits_in_window (bits_per_measure : Nat) (sched : List SysCall)
    (t0 window_us : Nat) : Nat :=
  sched.foldl (fun acc sc =>
    match sc.kind with
    | .Measure _ _ =>
        if decide (t0 ≤ sc.begin_us) && decide (sc.begin_us < t0 + window_us)
        then acc + bits_per_measure
        else acc
    | _ => acc) 0

/-- **I5 syndrome-bandwidth check.**  Every measurement-anchored window
...
def syndrome_bandwidth_ok (bits_per_measure window_us max_bits : Nat)
  ...
  let begin_times := (sched.filter (fun sc =>
    match sc.kind with
    | .Measure _ _ => true
    | _            => false)).map (·.begin_us)
  begin_times.all (fun t0 =>
    decide (syndrome_bits_in_window bits_per_measure sched t0 window_us
              ≤ max_bits))

/-! ## I6: space-time CAUSALITY (no resource counting — pure ordering)

    Two prefix-flow checks, both decidable:

    * **Syndrome causality** — syndrome data must EXIST before it is
      shipped and decoded: the n-th `DecodeSyndrome` (by begin time,
      ties counted conservatively) requires at least n `Measure`s whose
      intervals have ENDED by its begin.  A decode fired before its
      round's measurements finish — or more decodes than completed
      measurements — is causally invalid no matter how many decoders
      are free.

    * **Magic supply causality** — a magic state must be PREPARED before
      it is consumed: the n-th `RequestMagicState` requires
      `n ≤ initial_stock + ⌊t/period⌋·per_period` at its begin (the
      factory's production curve; `period = 0` means no production —
      stock only).  This is the causal counterpart of the I4 demand
      window: I4 caps the demand RATE, this orders consumption AFTER
      production.

    The FTQ-VM enforces the same two flows causally (syndrome tokens
    produced at measurement completion and consumed by decodes; magic
    tokens from stock/factories), so verdicts agree on the shared
    surface. -/
...
def measures_done_by (sched : List SysCall) (t : Nat) : Nat :=
  (sched.filter (fun sc =>
    match sc.kind with
    | .Measure _ _ => decide (sc.end_us ≤ t)
    | _            => false)).length
...
def decodes_begun_by (sched : List SysCall) (t : Nat) : Nat :=
  (sched.filter (fun sc =>
    match sc.kind with
    | .DecodeSyndrome _ => decide (sc.begin_us ≤ t)
    | _                 => false)).length
...
def syndrome_causality_ok (sched : List SysCall) : Bool :=
  sched.all fun sc =>
    match sc.kind with
    | .DecodeSyndrome _ =>
        decide (decodes_begun_by sched sc.begin_us
                  ≤ measures_done_by sched sc.begin_us)
    | _ => true
...
def magic_begun_by (sched : List SysCall) (t : Nat) : Nat :=
  (sched.filter (fun sc =>
    match sc.kind with
    | .RequestMagicState _ => decide (sc.begin_us ≤ t)
    | _                    => false)).length
...
def magic_available_by (initial period per t : Nat) : Nat :=
  if period = 0 then initial else initial + (t / period) * per
...
def magic_supply_ok (initial period per : Nat) (sched : List SysCall) : Bool :=
  sched.all fun sc =>
    match sc.kind with
    | .RequestMagicState _ =>
        decide (magic_begun_by sched sc.begin_us
                  ≤ magic_available_by initial period per sc.begin_us)
    | _ => true
...
end FormalRV.System.ScheduleInv
