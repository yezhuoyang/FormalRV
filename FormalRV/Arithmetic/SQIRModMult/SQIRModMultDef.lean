/-
  FormalRV.Arithmetic.SQIRModMult.SQIRModMultDef
  ──────────────────────────────────────────────
  THE definition of the SQIR-faithful in-place modular multiplier, as concrete
  `Gate`-IR data. **Definitions only — no proofs.**

  THE multiplier is `sqir_modmult_MCP_gate bits N a ainv`: it maps the data
  register `x ↦ (a · x) mod N` in place, on `sqir_total_dim bits` qubits, where
  ...
  Construction (bottom-up):
    ...
    + swap_acc_mult                -- swap accumulator ↔ multiplier register
    = inplace_candidate            -- x ↦ (a·x) mod N  (compute · swap · uncompute)
    + Gate.shift + encode adapter  = sqir_modmult_MCP_gate  (MultiplyCircuitProperty layout)

  Correctness : `SQIRModMultCorrectness.lean`  (`sqir_modmult_correct`)
  Resource    : `SQIRModMultResource.lean`     (`sqir_modmult_tcount = 112·bits²`)
...
def sqir_mult_control_idx (bits j : Nat) : Nat := 2 + (2 * bits + 1) + j
...
def sqir_swap_acc_mult (bits : Nat) : Gate := sqir_swap_acc_mult_aux bits bits
...
Correctness: `sqir_modmult_correct`.  Resource: `sqir_modmult_tcount = 112·bits²`. -/
