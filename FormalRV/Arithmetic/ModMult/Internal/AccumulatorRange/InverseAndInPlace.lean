...
theorem modmult_inplace_candidate_state_eq
  ...
  rw [modmult_const_gate_state_eq_from bits N a x 0 hbits hN_pos hN hN2 hN_pos hx_lt_pow]
  ...
  -- Now input is modmult_input_F bits ((a*x) % N) x.
  rw [modmult_const_gate_state_eq_from bits N ((N - ainv) % N) ((a * x) % N) x
        hbits hN_pos hN hN2 hx hax_lt_pow]
  -- Result: modmult_input_F bits ((a*x) % N) ((x + ((N - ainv) % N) * ((a*x) % N)) % N).
...
Built on:
- `modmult_const_gate_state_eq_from_qstart` (this file, L-3.15g).
- `modmult_swap_acc_mult_apply_qstart` (this file, L-3.15g.2 above).
- `modmult_inverse_clear_arith` (q_start-INDEPENDENT, above). -/

/-- q_start port of `modmult_inplace_candidate_state_eq`.
...
theorem modmult_inplace_candidate_state_eq_qstart
    ...
    Gate.applyNat
      ...
      = mult_input_F_qstart bits q_start ((a * x) % N) 0 := by
  unfold modmult_inplace_candidate_qstart
  simp only [Gate.applyNat_seq]
  -- Step 1: compute (x, 0) → (x, (a*x) % N).
  have hx_lt_pow : x < 2^bits := Nat.lt_of_lt_of_le hx hN
  rw [modmult_const_gate_state_eq_from_qstart bits q_start N a x 0 flagPos dim
  ...
  have hax_lt_N : (a * x) % N < N := Nat.mod_lt _ hN_pos
  have hax_lt_pow : (a * x) % N < 2^bits := Nat.lt_of_lt_of_le hax_lt_N hN
  rw [modmult_swap_acc_mult_apply_qstart bits q_start x ((a * x) % N) hbits
        hx_lt_pow hax_lt_pow]
  -- Step 3: uncompute with c = (N - ainv) % N.
  rw [modmult_const_gate_state_eq_from_qstart bits q_start N ((N - ainv) % N)
        ((a * x) % N) x flagPos dim
...
`q_start = 2` constants in `mult_control_idx bits 0 = 2*bits + 1`
...
`modmult_inplace_candidate_clean`, line 3733).  The bundle
...
/-- q_start port of `modmult_inplace_candidate_target_decode`
...
theorem modmult_inplace_candidate_target_decode_qstart
    ...
    cuccaro_target_val bits q_start
        (Gate.applyNat
          ...
          (mult_input_F_qstart bits q_start x 0))
      = 0 := by
  rw [modmult_inplace_candidate_state_eq_qstart bits q_start N a ainv x
        flagPos dim hbits hN_pos hN hN2 h_ainv_le hx h_inv
        h_flag_lt_qstart h_workspace h_dim_covers_mult]
  exact mult_input_target_decode_qstart bits q_start ((a * x) % N) 0
          (Nat.two_pow_pos bits)

/-- q_start port of `modmult_inplace_candidate_mult_bit` (line 3721):
...
theorem modmult_inplace_candidate_mult_bit_qstart
  ...
  rw [modmult_inplace_candidate_state_eq_qstart bits q_start N a ainv x
        flagPos dim hbits hN_pos hN hN2 h_ainv_le hx h_inv
        h_flag_lt_qstart h_workspace h_dim_covers_mult]
  exact mult_input_control_bit_qstart bits q_start ((a * x) % N) 0 k hk

/-- q_start port of `modmult_inplace_candidate_clean` (line 3733).
...
* multiplier-bit decoding at every `mult_control_idx_qstart bits q_start k`
...
theorem modmult_inplace_candidate_clean_qstart
    ...
    ∧ (∀ q, q < q_start →
        Gate.applyNat
          (modmult_inplace_candidate_qstart bits q_start N a ainv flagPos)
          (mult_input_F_qstart bits q_start x 0) q = false)
    ∧ Gate.applyNat
          (modmult_inplace_candidate_qstart bits q_start N a ainv flagPos)
          (mult_input_F_qstart bits q_start x 0) (q_start + 2 * bits) = false
    ∧ ∀ k, k < bits →
        Gate.applyNat
          (modmult_inplace_candidate_qstart bits q_start N a ainv flagPos)
          (mult_input_F_qstart bits q_start x 0)
          (mult_control_idx_qstart bits q_start k)
          = ((a * x) % N).testBit k := by
  have h_state := modmult_inplace_candidate_state_eq_qstart bits q_start N a ainv x
    flagPos dim hbits hN_pos hN hN2 h_ainv_le hx h_inv
    h_flag_lt_qstart h_workspace h_dim_covers_mult
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · rw [h_state]
    exact mult_input_target_decode_qstart bits q_start ((a * x) % N) 0
            (Nat.two_pow_pos bits)
  · rw [h_state]; exact mult_input_read_decode_qstart bits q_start ((a * x) % N) 0
  · intro q hq
    rw [h_state]
    exact mult_input_at_below_qstart_eq_false_qstart bits q_start
            ((a * x) % N) 0 q hq
  · rw [h_state]
    exact mult_input_top_carry_false_qstart bits q_start ((a * x) % N) 0 hbits
  · intro k hk
    rw [h_state]
    exact mult_input_control_bit_qstart bits q_start ((a * x) % N) 0 k hk
