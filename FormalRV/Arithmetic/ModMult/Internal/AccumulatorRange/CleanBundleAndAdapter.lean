...
theorem modmult_inplace_candidate_target_decode
    ...
    cuccaro_target_val bits 2
      ...
      = 0 := by
  rw [modmult_inplace_candidate_state_eq bits N a ainv x hbits hN_pos hN hN2 h_ainv_le hx h_inv]
  exact mult_input_target_decode bits ((a * x) % N) 0 (Nat.two_pow_pos bits)
...
theorem modmult_inplace_candidate_mult_bit
    ...
    Gate.applyNat (modmult_inplace_candidate bits N a ainv)
      ...
      = ((a * x) % N).testBit k := by
  rw [modmult_inplace_candidate_state_eq bits N a ainv x hbits hN_pos hN hN2 h_ainv_le hx h_inv]
  exact mult_input_control_bit bits ((a * x) % N) 0 k hk
...
theorem modmult_inplace_candidate_clean
    ...
    ∧ cuccaro_read_val bits 2
        (Gate.applyNat (modmult_inplace_candidate bits N a ainv)
          (modmult_input_F bits x 0)) = 0
    ∧ Gate.applyNat (modmult_inplace_candidate bits N a ainv)
          (modmult_input_F bits x 0) 0 = false
    ∧ Gate.applyNat (modmult_inplace_candidate bits N a ainv)
          (modmult_input_F bits x 0) 1 = false
    ∧ Gate.applyNat (modmult_inplace_candidate bits N a ainv)
          (modmult_input_F bits x 0) (2 + 2 * bits) = false
    ∧ ∀ k, k < bits →
        Gate.applyNat (modmult_inplace_candidate bits N a ainv)
          (modmult_input_F bits x 0) (mult_control_idx bits k)
          = ((a * x) % N).testBit k := by
  have h_state := modmult_inplace_candidate_state_eq bits N a ainv x
    hbits hN_pos hN hN2 h_ainv_le hx h_inv
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · rw [h_state]; exact mult_input_target_decode bits ((a * x) % N) 0 (Nat.two_pow_pos bits)
  · rw [h_state]; exact mult_input_read_decode bits ((a * x) % N) 0
  · rw [h_state]; exact mult_input_flag_0_false bits ((a * x) % N) 0
  · rw [h_state]; exact mult_input_flag_1_false bits ((a * x) % N) 0
  · rw [h_state]; exact mult_input_top_carry_false bits ((a * x) % N) 0 hbits
  · intro k hk
    rw [h_state]
    exact mult_input_control_bit bits ((a * x) % N) 0 k hk
...
theorem mult_input_F_shifted_below_bits
    ...
    mult_input_F_shifted bits x acc q = false := by
  unfold mult_input_F_shifted; rw [if_pos hq]

theorem mult_input_F_shifted_above_bits
    (bits x acc q : Nat) (hq : bits ≤ q) :
    mult_input_F_shifted bits x acc q
      = modmult_input_F bits x acc (q - bits) := by
  unfold mult_input_F_shifted; rw [if_neg (Nat.not_lt.mpr hq)]
...
theorem mult_input_F_shifted_at_shifted_control_bit
    ...
    mult_input_F_shifted bits x acc (bits + mult_control_idx bits k)
      = x.testBit k := by
  rw [mult_input_F_shifted_above_bits bits x acc _ (by omega)]
  rw [show bits + mult_control_idx bits k - bits = mult_control_idx bits k from by omega]
  exact mult_input_control_bit bits x acc k hk
...
theorem encode_to_mult_adapter_disjoint (bits : Nat) :
    0 + bits ≤ bits + mult_control_idx bits 0
      ∨ bits + mult_control_idx bits 0 + bits ≤ 0 := by
  left; unfold mult_control_idx; omega

theorem encode_to_mult_adapter_wellTyped
    (bits : Nat) (hbits : 1 ≤ bits) :
    Gate.WellTyped (modmult_total_dim bits) (encode_to_mult_adapter bits) := by
  unfold encode_to_mult_adapter modmult_total_dim
  apply reverse_register_swap_wellTyped
  · unfold sqir_modmult_rev_anc; omega
  · unfold sqir_modmult_rev_anc; omega
  · unfold mult_control_idx sqir_modmult_rev_anc; omega
  · exact encode_to_mult_adapter_disjoint bits
...
theorem encode_to_mult_adapter_correct
    ...
    Gate.applyNat (encode_to_mult_adapter bits)
      ...
      = mult_input_F_shifted bits x 0 := by
  funext q
  unfold encode_to_mult_adapter
  set offB := bits + mult_control_idx bits 0 with h_offB_def
  have h_offB_val : offB = 2 + 2 * bits + 1 + bits := by
    rw [h_offB_def]; unfold mult_control_idx; omega
  have h_disjoint : 0 + bits ≤ offB ∨ offB + bits ≤ 0 :=
    encode_to_mult_adapter_disjoint bits
  ...
  · push_neg at hq_lo
    ...
    · obtain ⟨h_q_ge, h_q_lt⟩ := hq_in_mult
      ...
      have h_q_minus_bits : q - bits = mult_control_idx bits (q - offB) := by
        rw [h_offB_val] at h_q_ge h_q_lt; unfold mult_control_idx; omega
      rw [mult_input_F_shifted_above_bits bits x 0 q (by omega)]
      rw [h_q_minus_bits]
      rw [mult_input_control_bit bits x 0 (q - offB) (by omega)]
      -- LHS:
    ...
    · -- Other positions: identity.
      ...
      unfold reverse_register_swap
      rw [h_lhs_id]
      rw [mult_input_F_shifted_above_bits bits x 0 q hq_lo]
      ...
      · push_neg at hq_below_offB
        ...
        have h_RHS_false : modmult_input_F bits x 0 (q - bits) = false := by
          unfold modmult_input_F
...
theorem encode_to_mult_adapter_involution
    (bits : Nat) (f : Nat → Bool) :
    Gate.applyNat (encode_to_mult_adapter bits)
      (Gate.applyNat (encode_to_mult_adapter bits) f) = f := by
  unfold encode_to_mult_adapter
  exact reverse_register_swap_involution_general bits 0 _
    (encode_to_mult_adapter_disjoint bits) f
...
theorem modmult_inplace_shifted_wellTyped
    ...
    Gate.WellTyped (modmult_total_dim bits) (modmult_inplace_shifted bits N a ainv) := by
  unfold modmult_inplace_shifted modmult_total_dim
  apply Gate.shift_wellTyped
  -- Need: WellTyped (sqir_modmult_rev_anc bits) (modmult_inplace_candidate bits N a ainv).
  ...
  · exact modmult_swap_acc_mult_wellTyped bits hbits
  · exact modmult_prefix_gate_wellTyped bits N ((N - ainv) % N) bits
...
theorem modmult_MCP_gate_apply_encode
  ...
  rw [encode_to_mult_adapter_correct bits x hbits hx_lt_pow]
  rw [modmult_inplace_shifted_correct bits N a ainv x hbits hN_pos hN hN2 h_ainv_le hx h_inv]
  have h_ax_lt_N : (a * x) % N < N := Nat.mod_lt _ hN_pos
  have h_ax_lt_pow : (a * x) % N < 2^bits := Nat.lt_of_lt_of_le h_ax_lt_N hN
  exact encode_to_mult_adapter_reverse bits ((a * x) % N) hbits h_ax_lt_pow

/-- **MCP-layout gate WellTyped.** -/
theorem modmult_MCP_gate_wellTyped
    (bits N a ainv : Nat) (hbits : 1 ≤ bits) (hN_pos : 0 < N)
    (hN : N ≤ 2^bits) (hN2 : 2 * N ≤ 2^bits) :
    Gate.WellTyped (modmult_total_dim bits) (modmult_MCP_gate bits N a ainv) := by
  unfold modmult_MCP_gate
  refine ⟨?_, ?_, ?_⟩
  · exact encode_to_mult_adapter_wellTyped bits hbits
...
theorem modmult_MCP_gate_satisfies_MultiplyCircuitProperty
    ...
    FormalRV.SQIRPort.MultiplyCircuitProperty a N bits (sqir_modmult_rev_anc bits)
      (Gate.toUCom (modmult_total_dim bits) (modmult_MCP_gate bits N a ainv)) := by
  ...
  apply toUCom_satisfies_MultiplyCircuitProperty_of_applyNat_encodeDataZeroAnc
    ...
    hN
  intro x hx
  exact modmult_MCP_gate_apply_encode bits N a ainv x hbits hN_pos hN hN2 h_ainv_le hx h_inv
