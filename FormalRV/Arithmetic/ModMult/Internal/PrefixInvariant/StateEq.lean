...
After applying the step gate to `modmult_input_F bits m acc`, the
state is exactly `modmult_input_F bits m acc'` where
...
theorem modmult_step_state_eq
  ...
  · -- Above-layout case.
    rw [modmult_step_above_layout_false bits N a j m acc q hbits hj hq_above]
    -- RHS: modmult_input_F bits m acc' q = false (above layout).
  ...
  · push_neg at hq_above
    ...
    · -- Multiplier register.  q = mult_control_idx bits k where k = q - (2 + 2*bits + 1).
      ...
      rw [hq_eq]
      rw [modmult_step_preserves_all_control_bits bits N a m acc j k
            hbits hN_pos hN hN2 hacc hj hk_lt]
      exact (mult_input_control_bit bits m acc' k hk_lt).symm
    · push_neg at hq_in_mult
      -- Workspace: q < 2 + 2*bits + 1.
      -- Sub-cases on q.
      by_cases hq_0 : q = 0
      · subst hq_0
        rw [modmult_step_flag0_false bits N a j m acc hbits hj]
        exact (mult_input_flag_0_false bits m acc').symm
      by_cases hq_1 : q = 1
      · subst hq_1
        have ⟨_, _, h_fl, _⟩ :=
          modmult_step_workspace bits N a j m acc hbits hN_pos hN hN2 hj hacc
        rw [h_fl]
        exact (mult_input_flag_1_false bits m acc').symm
      by_cases hq_2 : q = 2
      · subst hq_2
        rw [modmult_step_carry_in_restored bits N a j m acc hbits hN_pos hN hN2 hj hacc]
        -- RHS: modmult_input_F bits m acc' 2 = cuccaro_input_F at q_start = false.
        unfold modmult_input_F
      ...
      · subst hq_top
        have ⟨_, h_tc, _, _⟩ :=
          modmult_step_workspace bits N a j m acc hbits hN_pos hN hN2 hj hacc
        rw [h_tc]
        -- RHS: modmult_input_F bits m acc' (2 + 2*bits)
      ...
      · -- q odd: q = 2 + 2*i + 1 for i = (q-3)/2 < bits.
        ...
        rw [hq_eq]
        rw [modmult_step_target_bit bits N a j m acc ((q - 3) / 2)
      ...
      · -- q even: q = 2 + 2*i + 2 for i = (q-4)/2 < bits.
        ...
        rw [hq_eq]
        rw [modmult_step_read_bit bits N a j m acc ((q - 4) / 2)
              hbits hN_pos hN hN2 hj hacc hi_lt]
        unfold modmult_input_F
...
`funext q` case split mirrors the hard-coded `modmult_step_state_eq`
...
theorem modmult_step_state_eq_qstart
  ...
  · -- Above layout.
    rw [modmult_step_above_layout_false_qstart bits q_start N a j flagPos m acc q
  ...
  · push_neg at hq_above
    ...
    · -- Multiplier register.
      ...
      rw [hq_eq]
      rw [modmult_step_preserves_all_control_bits_qstart bits q_start N a m acc j k flagPos
            hbits hN_pos hN hN2 hacc hj hk_lt h_flag_lt_qstart dim h_workspace h_dim_covers_mult]
      exact (mult_input_control_bit_qstart bits q_start m acc' k hk_lt).symm
    · push_neg at hq_in_mult
      ...
      · -- Below the Cuccaro workspace.
        ...
        · -- q = flagPos: use workspace_qstart flag conjunct.
          ...
          rw [h_fl]
          -- RHS: mult_input_F_qstart at flagPos = false (workspace branch, q < q_start).
          unfold mult_input_F_qstart
        ...
        · -- q ≠ flagPos and q < q_start: use untouched_pos.
          ...
          have h_q_ne_ctrl_j : q ≠ mult_control_idx_qstart bits q_start j := by
            unfold mult_control_idx_qstart; omega
          have h_input_false : mult_input_F_qstart bits q_start m acc q = false := by
            unfold mult_input_F_qstart
            rw [if_pos (by omega : q < q_start + 2 * bits + 1)]
            unfold cuccaro_input_F
            rw [if_pos hq_below]
          rw [modmult_step_at_untouched_pos_qstart bits q_start N a j flagPos m acc q
                hj h_input_false h_q_out hq_flag h_q_ne_ctrl_j]
          -- RHS: mult_input_F_qstart at q (with acc') = false.
          unfold mult_input_F_qstart
          rw [if_pos (by omega : q < q_start + 2 * bits + 1)]
          unfold cuccaro_input_F
          rw [if_pos hq_below]
      · -- q ≥ q_start, q < q_start + 2 * bits + 1.  Cuccaro workspace.
        ...
        · -- carry-in.
          rw [hq_qs]
          rw [modmult_step_carry_in_restored_qstart bits q_start N a j flagPos m acc
        ...
        · by_cases hq_top : q = q_start + 2 * bits
          ...
          · -- q ∈ (q_start, q_start + 2*bits).  Parity dispatch on (q - q_start).
            ...
            · -- Odd offset: q = q_start + 2*i + 1 with i = (q - q_start - 1) / 2.
              ...
              rw [hq_eq]
              rw [modmult_step_target_bit_qstart bits q_start N a j flagPos m acc
            ...
            · -- Even offset: q = q_start + 2*i + 2 with i = (q - q_start - 2) / 2.
              ...
              rw [hq_eq]
              rw [modmult_step_read_bit_qstart bits q_start N a j flagPos m acc
...
By induction on `k`, the prefix gate's output on `modmult_input_F bits m 0`
equals `modmult_input_F bits m (acc_spec ... k)`.  Uses
`modmult_step_state_eq` at each step + the accumulator recurrence. -/
theorem modmult_prefix_state_eq
    ...
    Gate.applyNat (modmult_prefix_gate bits N a k) (modmult_input_F bits m 0)
      = modmult_input_F bits m (modmult_acc_spec N a m k) := by
  induction k with
  | zero =>
    rw [modmult_prefix_gate, Gate.applyNat_I, modmult_acc_spec_zero]
  | succ n ih =>
    have hn_le : n ≤ bits := by omega
    have hn_lt : n < bits := by omega
    rw [modmult_prefix_gate_succ_eq, Gate.applyNat_seq]
    rw [ih hn_le]
    -- Now: applyNat (step n) (modmult_input_F bits m (acc_spec n))
    --    = modmult_input_F bits m (acc_spec (n+1))
    have hacc_lt_N : modmult_acc_spec N a m n < N :=
      modmult_acc_spec_lt N a m n hN_pos
    rw [modmult_step_state_eq bits N a n m (modmult_acc_spec N a m n)
...
theorem modmult_prefix_target_decode
    ...
    cuccaro_target_val bits 2
        (Gate.applyNat (modmult_prefix_gate bits N a k) (modmult_input_F bits m 0))
      = modmult_acc_spec N a m k := by
  rw [modmult_prefix_state_eq bits N a m k hbits hN_pos hN hN2 hk]
  have h_lt : modmult_acc_spec N a m k < 2 ^ bits :=
    Nat.lt_of_lt_of_le (modmult_acc_spec_lt N a m k hN_pos) hN
  exact mult_input_target_decode bits m (modmult_acc_spec N a m k) h_lt

/-! ## Tick 76 — Deliverable G: Full modular multiplier target theorem. -/

/-- **Deliverable G — Full modular multiplier target theorem.**

After applying `modmult_const_gate bits N a` to
`modmult_input_F bits m 0`, the target register decodes to `(a*m) % N`. -/
theorem modmult_const_gate_target_decode
  ...
  rw [modmult_prefix_target_decode bits N a m bits hbits hN_pos hN hN2 (le_refl _)]
  exact modmult_acc_spec_eq_mul_mod bits N a m hN_pos hm
...
- a one-line q_start port of `mult_input_target_decode` (line
...
- a prefix state-equality (`_prefix_state_eq_qstart`) by induction on
  ...
  q_start-INDEPENDENT `modmult_acc_spec` recurrence;
...
`mult_input_target_decode` (line 115). -/
theorem mult_input_target_decode_qstart
    (bits q_start m acc : Nat) (hacc : acc < 2 ^ bits) :
    cuccaro_target_val bits q_start (mult_input_F_qstart bits q_start m acc) = acc := by
  have h_eq : cuccaro_target_val bits q_start (mult_input_F_qstart bits q_start m acc)
                = acc % 2 ^ bits := by
...
`modmult_prefix_state_eq` (line 2416). -/
theorem modmult_prefix_state_eq_qstart
    ...
    Gate.applyNat (modmult_prefix_gate_qstart bits q_start N a flagPos k)
      ...
      = mult_input_F_qstart bits q_start m (modmult_acc_spec N a m k) := by
  induction k with
  | zero =>
    rw [modmult_prefix_gate_qstart, Gate.applyNat_I, modmult_acc_spec_zero]
  | succ n ih =>
    have hn_le : n ≤ bits := by omega
    have hn_lt : n < bits := by omega
...
state equality).  Port of `modmult_prefix_target_decode`
(line 2443). -/
theorem modmult_prefix_target_decode_qstart
    ...
    cuccaro_target_val bits q_start
        (Gate.applyNat (modmult_prefix_gate_qstart bits q_start N a flagPos k)
          (mult_input_F_qstart bits q_start m 0))
      = modmult_acc_spec N a m k := by
  rw [modmult_prefix_state_eq_qstart bits q_start N a m k flagPos dim
        hbits hN_pos hN hN2 hk h_flag_lt_qstart h_workspace h_dim_covers_mult]
  have h_lt : modmult_acc_spec N a m k < 2 ^ bits :=
    Nat.lt_of_lt_of_le (modmult_acc_spec_lt N a m k hN_pos) hN
  exact mult_input_target_decode_qstart bits q_start m
    (modmult_acc_spec N a m k) h_lt

/-- **R7d^xxix-L-3.15e.5 HEADLINE: q_start-parametric full modular
multiplier target decode.**

After applying `modmult_const_gate_qstart bits q_start N a
flagPos` to `mult_input_F_qstart bits q_start m 0`, the target
register decodes to `(a * m) % N`.  Port of
`modmult_const_gate_target_decode` (line 2461). -/
theorem modmult_const_gate_target_decode_qstart
  ...
  rw [modmult_prefix_target_decode_qstart bits q_start N a m bits flagPos dim
