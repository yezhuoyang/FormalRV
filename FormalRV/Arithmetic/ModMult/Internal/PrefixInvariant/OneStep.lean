...
theorem modmult_prefix_target_decode_zero
    ...
    cuccaro_target_val bits 2
      ...
      = modmult_acc_spec N a m 0 := by
  rw [modmult_acc_spec_zero, modmult_prefix_gate]
  rw [Gate.applyNat_I]
  exact mult_input_target_decode bits m 0 (Nat.two_pow_pos bits)
    ...
    · have h_out : mult_control_idx bits k < 2
      ...
      have h_lhs_eq : install_mult_bits_skip_j bits m j (k+1) f
                    = update (install_mult_bits_skip_j bits m j k f)
                        (mult_control_idx bits k) (m.testBit k) := by
        show (if k = j then install_mult_bits_skip_j bits m j k f
              else update (install_mult_bits_skip_j bits m j k f)
                          (mult_control_idx bits k) (m.testBit k))
            = update (install_mult_bits_skip_j bits m j k f)
...
The one-step gate `modmult_step_gate bits N a j` preserves every
...
consumed by `modmult_step_state_eq` (deferred to Tick 76). -/
...
theorem modmult_step_state_normal
    ...
    ∧ ∀ k, k < bits →
        Gate.applyNat (modmult_step_gate bits N a j)
          ...
          = m.testBit k := by
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · exact modmult_step_target_decode bits N a j m acc hbits hN_pos hN hN2 hj hacc
  · have ⟨h_rd, _, _, _⟩ :=
      modmult_step_workspace bits N a j m acc hbits hN_pos hN hN2 hj hacc
    exact h_rd
  · have ⟨_, _, h_fl, _⟩ :=
  ...
  · have ⟨_, h_tc, _, _⟩ :=
    ...
    exact h_tc
  · intro k hk
    exact modmult_step_preserves_all_control_bits bits N a m acc j k
...
theorem modmult_acc_spec_eq_mul_mod_pow
  ...
  | succ n ih =>
    ...
    · have h_bit_false : m.testBit n = false := by
      ...
      rw [modmult_acc_spec_succ_false N a m n h_bit_false]
      rw [h_bit_false]
...
theorem modmult_acc_spec_eq_mul_mod
    ...
    modmult_acc_spec N a m bits = (a * m) % N := by
  rw [modmult_acc_spec_eq_mul_mod_pow N a m bits hN_pos]
