...
`modmult_step_state_eq_qstart` proof.  The above-layout and
...
`modmult_step_at_untouched_pos` (line 1712). -/
theorem modmult_step_at_untouched_pos_qstart
  ...
  have h_in_eq : update (mult_input_F_qstart bits q_start m acc) q false
                = mult_input_F_qstart bits q_start m acc := by
    rw [show (false : Bool) = mult_input_F_qstart bits q_start m acc q from h_input.symm]
    exact update_self _ q
  unfold modmult_step_gate_qstart
  have h_commute := style_controlledModAddConst_gate_commute_update_outside_fun_qstart
    bits q_start N ((a * 2^j) % N)
    (mult_control_idx_qstart bits q_start j) flagPos q false
...
`false`.  Port of `modmult_step_flag0_false` (line 1734). -/
theorem modmult_step_flag0_false_qstart
  ...
  apply modmult_step_at_untouched_pos_qstart bits q_start N a j flagPos m acc 0 hj
  · exact mult_input_flag_0_false_qstart bits q_start m acc (by omega)
  · exact Or.inl (by omega)
  · exact h_flag_ne_0
  · have h := mult_control_idx_outside_modadd_workspace_form_qstart bits q_start j
    intro h_eq; unfold mult_control_idx_qstart at h_eq; omega

/-- q_start-parametric: step gate's output above the multiplier
register (for `q ≥ q_start + 2 * bits + 1 + bits`) is `false`.  Port
of `modmult_step_above_layout_false` (line 1747). -/
theorem modmult_step_above_layout_false_qstart
    ...
    Gate.applyNat (modmult_step_gate_qstart bits q_start N a j flagPos)
        (mult_input_F_qstart bits q_start m acc) q = false := by
  apply modmult_step_at_untouched_pos_qstart bits q_start N a j flagPos m acc q hj
  ...
  · intro h_eq
    have h := mult_control_idx_outside_modadd_workspace_form_qstart bits q_start j
    unfold mult_control_idx_qstart at h_eq
    omega
...
`style_modAddConst_clean_candidate_carry_in_restored`
(line 1637). -/
...
`style_controlledModAddConst_candidate_carry_in_restored`
(line 1657). -/
theorem style_controlledModAddConst_candidate_carry_in_restored_qstart
  ...
  | true =>
    rw [sqir_style_controlledModAddConst_candidate_control_true_state_eq_qstart
          ...
          hcontrol_out hflag_out hcontrol_ne_flag]
    rw [update_neq _ _ _ _ (Ne.symm h_ctrl_ne_qs)]
    exact style_modAddConst_clean_candidate_carry_in_restored_qstart bits q_start N c x
...
`style_controlledModAddConst_gate_carry_in_restored` (line 1684). -/
theorem style_controlledModAddConst_gate_carry_in_restored_qstart
  ...
  · have hc_pos : 0 < c := Nat.pos_of_ne_zero hc0
    ...
    exact style_controlledModAddConst_candidate_carry_in_restored_qstart bits q_start N c x
...
`style_controlledModAddConst_gate_commute_install` (line 1362). -/
...
(`q_start`) is `false`.  Port of `modmult_step_carry_in_restored`
(line 1764). -/
theorem modmult_step_carry_in_restored_qstart
    ...
    (h_flag_lt_qstart : flagPos < q_start) :
    Gate.applyNat (modmult_step_gate_qstart bits q_start N a j flagPos)
        (mult_input_F_qstart bits q_start m acc) q_start = false := by
  ...
  exact style_controlledModAddConst_gate_carry_in_restored_qstart bits q_start N
    ((a * 2^j) % N) acc (mult_control_idx_qstart bits q_start j) flagPos (m.testBit j)
