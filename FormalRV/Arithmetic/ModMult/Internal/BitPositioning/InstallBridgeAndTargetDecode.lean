...
/-- **`modmult_input_F` decomposes as `install_mult_bits_skip_j` applied
...
theorem mult_input_F_eq_install_with_j
    ...
    modmult_input_F bits m acc
      = install_mult_bits_skip_j bits m j bits
          (update (cuccaro_input_F 2 false 0 acc) (mult_control_idx bits j) (m.testBit j)) := by
  funext q
  ...
  · -- q < workspace upper bound: both sides = cuccaro_input_F at q.
    ...
    -- LHS: modmult_input_F q = cuccaro_input_F at q.
    have h_lhs : modmult_input_F bits m acc q = cuccaro_input_F 2 false 0 acc q := by
      unfold modmult_input_F; rw [if_pos hq_ws]
    -- RHS: update F controlIdx_j _ at q = F at q (since q < controlIdx_j).
    have h_q_ne_ctrl_j : q ≠ mult_control_idx bits j := by
      have : mult_control_idx bits j ≥ 2 + 2 * bits + 1 := by
  ...
  · push_neg at hq_ws
    ...
    · -- q is in multiplier register.
      ...
      have h_q_eq : q = mult_control_idx bits k := by
        unfold mult_control_idx; omega
      rw [h_q_eq]
      have h_lhs : modmult_input_F bits m acc (mult_control_idx bits k)
                 = m.testBit k :=
        mult_input_control_bit bits m acc k hk_lt
      rw [h_lhs]
      by_cases h_kj : k = j
    ...
    · push_neg at hq_in_mult
      ...
      have h_lhs : modmult_input_F bits m acc q = false := by
        unfold modmult_input_F
        rw [if_neg (by omega : ¬ q < 2 + 2 * bits + 1)]
        rw [if_neg (by omega : ¬ q < 2 + 2 * bits + 1 + bits)]
      have h_q_ne_ctrl_j : q ≠ mult_control_idx bits j := by
        have : mult_control_idx bits j < 2 + 2 * bits + 1 + bits := by
          unfold mult_control_idx; omega
        omega
      rw [h_lhs, update_neq _ _ _ _ h_q_ne_ctrl_j]
...
theorem install_mult_bits_skip_j_at_workspace_eq_qstart
  ...
  | succ k ih =>
    ...
    · rw [if_neg h_eq]
      have h_ne : q ≠ mult_control_idx_qstart bits q_start k := by
        have h_ctrl_ge :
            mult_control_idx_qstart bits q_start k ≥ q_start + 2 * bits + 1 := by
  ...
  | succ n ih =>
    ...
    · rw [if_neg h_eq]
      ...
      · have h_ne :
          ...
          intro heq
          exact h_kn (mult_control_idx_injective_qstart bits q_start k n heq)
        rw [update_neq _ _ _ _ h_ne]
        apply ih
        omega
...
theorem install_mult_bits_skip_j_at_j_eq_qstart
    ...
    install_mult_bits_skip_j_qstart bits q_start m j num_bits f
      ...
      = f (mult_control_idx_qstart bits q_start j) := by
  induction num_bits with
  | zero => rfl
  | succ k ih =>
    unfold install_mult_bits_skip_j_qstart
    ...
    · rw [if_neg h_kj]
      have h_ne :
          mult_control_idx_qstart bits q_start j
            ≠ mult_control_idx_qstart bits q_start k :=
        fun heq => h_kj
          (mult_control_idx_injective_qstart bits q_start j k heq).symm
      rw [update_neq _ _ _ _ h_ne]
      exact ih
...
theorem install_mult_bits_skip_j_at_above_eq_qstart
  ...
  | succ k ih =>
    ...
    · rw [if_neg h_eq]
      have h_ne : q ≠ mult_control_idx_qstart bits q_start k := by
        have h_ctrl_lt :
            mult_control_idx_qstart bits q_start k < q_start + 2 * bits + 1 + bits := by
          unfold mult_control_idx_qstart; omega
        omega
      rw [update_neq _ _ _ _ h_ne]
      exact ih (by omega)

/-- q_start-parametric bridge: `mult_input_F_qstart bits q_start m
acc` decomposes as `install_mult_bits_skip_j_qstart` applied to
`update (cuccaro_input_F q_start false 0 acc) (mult_control_idx_qstart
bits q_start j) (m.testBit j)`.  Port of
`mult_input_F_eq_install_with_j` (line 544). -/
theorem mult_input_F_eq_install_with_j_qstart
  ...
  · -- q below workspace upper bound: both sides equal `cuccaro_input_F` at q.
    ...
    have h_lhs : mult_input_F_qstart bits q_start m acc q
                = cuccaro_input_F q_start false 0 acc q := by
      unfold mult_input_F_qstart; rw [if_pos hq_ws]
    have h_q_ne_ctrl_j : q ≠ mult_control_idx_qstart bits q_start j := by
      have : mult_control_idx_qstart bits q_start j ≥ q_start + 2 * bits + 1 := by
        unfold mult_control_idx_qstart; omega
      omega
    rw [h_lhs, update_neq _ _ _ _ h_q_ne_ctrl_j]
  · push_neg at hq_ws
    ...
    · -- q is in multiplier register.
      ...
      have hk_lt : k < bits := by omega
      have h_q_eq : q = mult_control_idx_qstart bits q_start k := by
        unfold mult_control_idx_qstart; omega
      rw [h_q_eq]
      have h_lhs : mult_input_F_qstart bits q_start m acc
                      (mult_control_idx_qstart bits q_start k)
                 = m.testBit k :=
        mult_input_control_bit_qstart bits q_start m acc k hk_lt
      rw [h_lhs]
      by_cases h_kj : k = j
    ...
    · push_neg at hq_in_mult
      ...
      rw [install_mult_bits_skip_j_at_above_eq_qstart bits q_start m j bits (le_refl _) _ q
            hq_in_mult]
      have h_lhs : mult_input_F_qstart bits q_start m acc q = false := by
        unfold mult_input_F_qstart
        rw [if_neg (by omega : ¬ q < q_start + 2 * bits + 1)]
        rw [if_neg (by omega : ¬ q < q_start + 2 * bits + 1 + bits)]
      have h_q_ne_ctrl_j : q ≠ mult_control_idx_qstart bits q_start j := by
        have : mult_control_idx_qstart bits q_start j < q_start + 2 * bits + 1 + bits := by
          unfold mult_control_idx_qstart; omega
        omega
      rw [h_lhs, update_neq _ _ _ _ h_q_ne_ctrl_j]
...
After applying `modmult_step_gate bits N a j` to
`modmult_input_F bits m acc`, the decoded target register equals
...
theorem modmult_step_target_decode
  ...
  -- Bridge modmult_input_F to install over F_j (Deliverable B).
