...
theorem mult_control_idx_outside_modadd_workspace
    ...
    mult_control_idx bits j < 2
      ∨ 2 + (2 * bits + 1) ≤ mult_control_idx bits j := by
  right
  unfold mult_control_idx
  omega

theorem mult_control_idx_ne_flag
    (bits j : Nat) :
    mult_control_idx bits j ≠ 1 := by
  unfold mult_control_idx
  omega

theorem mult_control_idx_ne_top_carry
    (bits j : Nat) :
    mult_control_idx bits j ≠ 2 + 2 * bits := by
  unfold mult_control_idx
  omega

theorem mult_control_idx_lt_sqir_dim
    (bits j : Nat) (hj : j < bits) :
    mult_control_idx bits j < sqir_modmult_rev_anc bits := by
  unfold mult_control_idx sqir_modmult_rev_anc
  omega
...
theorem mult_input_control_bit
  ...
  unfold modmult_input_F mult_control_idx
...
theorem mult_input_target_decode
    ...
    cuccaro_target_val bits 2 (modmult_input_F bits m acc) = acc := by
  have h_eq : cuccaro_target_val bits 2 (modmult_input_F bits m acc) = acc % 2 ^ bits := by
...
theorem mult_input_read_decode
    (bits m acc : Nat) :
    cuccaro_read_val bits 2 (modmult_input_F bits m acc) = 0 := by
  have h_eq : cuccaro_read_val bits 2 (modmult_input_F bits m acc) = 0 % 2 ^ bits := by
...
theorem mult_input_flag_1_false
    (bits m acc : Nat) :
    modmult_input_F bits m acc 1 = false := by
  unfold modmult_input_F
...
theorem mult_input_top_carry_false
    ...
    modmult_input_F bits m acc (2 + 2 * bits) = false := by
  unfold modmult_input_F
...
theorem modmult_acc_spec_zero (N a m : Nat) :
    modmult_acc_spec N a m 0 = 0 := rfl

theorem modmult_acc_spec_succ_false
    (N a m k : Nat) (h : m.testBit k = false) :
    modmult_acc_spec N a m (k + 1) = modmult_acc_spec N a m k := by
  show (if m.testBit k then _ else modmult_acc_spec N a m k) = _
  simp [h]

theorem modmult_acc_spec_succ_true
    (N a m k : Nat) (h : m.testBit k = true) :
    modmult_acc_spec N a m (k + 1)
      = (modmult_acc_spec N a m k + (a * 2 ^ k) % N) % N := by
  show (if m.testBit k then (modmult_acc_spec N a m k + (a * 2 ^ k) % N) % N else _) = _
  simp [h]

/-- For `0 < N`, the accumulator after any prefix is in `[0, N)`. -/
theorem modmult_acc_spec_lt (N a m k : Nat) (hN_pos : 0 < N) :
    modmult_acc_spec N a m k < N := by
  induction k with
  | zero => exact hN_pos
  | succ n ih =>
    unfold modmult_acc_spec
...
theorem modmult_prefix_gate_zero_eq_I
    (bits N a : Nat) :
    modmult_prefix_gate bits N a 0 = Gate.I := rfl

theorem modmult_prefix_gate_succ_eq
    (bits N a k : Nat) :
    modmult_prefix_gate bits N a (k + 1)
      = seq (modmult_prefix_gate bits N a k) (modmult_step_gate bits N a k) := rfl
...
theorem style_controlledModAddConst_gate_commute_update_outside_fun
  ...
  · simp only [hc, if_false]
    ...
    rw [sqir_conditionalSubConstGate_commute_update_outside_fun bits 2 N 1 updateIdx v _
          hupdate_out hupdate_ne_flag]
    rw [controlledCompareConst_commute_update_outside_fun bits 2 c controlIdx 1 updateIdx v _
...
theorem mult_control_idx_injective_qstart
    ...
    j = j' := by
  unfold mult_control_idx_qstart at h
  omega

/-- The multiplier bit at `mult_control_idx_qstart bits q_start j`
is `m.testBit j`.  Port of `mult_input_control_bit` (line 103). -/
theorem mult_input_control_bit_qstart
  ...
  unfold mult_input_F_qstart mult_control_idx_qstart
...
Port of `style_controlledModAddConst_gate_commute_update_outside_fun`
...
- `controlledCompareConst_commute_update_outside_fun` (this file:249);
...
theorem style_controlledModAddConst_gate_commute_update_outside_fun_qstart
  ...
  · simp only [hc, if_false]
    ...
    rw [sqir_conditionalSubConstGate_commute_update_outside_fun bits q_start N flagPos
          updateIdx v _ hupdate_out hupdate_ne_flag]
    rw [controlledCompareConst_commute_update_outside_fun bits q_start c controlIdx flagPos
...
theorem install_mult_bits_skip_j_at_workspace_eq
  ...
  | succ k ih =>
    ...
    · rw [if_neg h_eq]
      have h_ne : q ≠ mult_control_idx bits k := by
        have h_ctrl_ge : mult_control_idx bits k ≥ 2 + 2 * bits + 1 := by
...
theorem install_mult_bits_skip_j_at_mult_k_eq
  ...
  | succ n ih =>
    ...
    · rw [if_neg h_eq]
      by_cases h_kn : k = n
      · rw [h_kn, update_eq]
      · have h_ne : mult_control_idx bits k ≠ mult_control_idx bits n := by
          intro heq
          exact h_kn (mult_control_idx_injective bits k n heq)
        rw [update_neq _ _ _ _ h_ne]
        apply ih
        omega
...
theorem install_mult_bits_skip_j_at_j_eq
  ...
  | succ k ih =>
    ...
    · rw [if_neg h_kj]
      have h_ne : mult_control_idx bits j ≠ mult_control_idx bits k :=
        fun heq => h_kj (mult_control_idx_injective bits j k heq).symm
      rw [update_neq _ _ _ _ h_ne]
      exact ih
...
theorem install_mult_bits_skip_j_at_above_eq
  ...
  | succ k ih =>
    ...
    · rw [if_neg h_eq]
      have h_ne : q ≠ mult_control_idx bits k := by
        have h_ctrl_lt : mult_control_idx bits k < 2 + 2 * bits + 1 + bits := by
          unfold mult_control_idx; omega
        omega
      rw [update_neq _ _ _ _ h_ne]
      exact ih (by omega)
