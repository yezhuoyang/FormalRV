...
and the headline `modmult_step_target_decode`.  Uses the L-3.15a
...
(`modmult_step_workspace_qstart`) is intentionally deferred. -/
...
theorem cuccaro_target_val_through_install_mult_qstart
    ...
    cuccaro_target_val bits q_start
      ...
      = cuccaro_target_val bits q_start
          (Gate.applyNat (sqir_style_controlledModAddConst_gate bits q_start N c
            (mult_control_idx_qstart bits q_start j) flagPos) f) := by
  ...
  | succ k ih =>
    ...
    · rw [if_pos h_kj]
      exact ih
    · rw [if_neg h_kj]
      have h_out : mult_control_idx_qstart bits q_start k < q_start
            ∨ q_start + 2 * bits + 1 ≤ mult_control_idx_qstart bits q_start k :=
        mult_control_idx_outside_modadd_workspace_form_qstart bits q_start k
      have h_ne_flag : mult_control_idx_qstart bits q_start k ≠ flagPos :=
        mult_control_idx_ne_flag_qstart bits q_start k flagPos h_flag_lt_qstart
      have h_ne_ctrl_j :
          mult_control_idx_qstart bits q_start k
            ≠ mult_control_idx_qstart bits q_start j :=
        fun heq => h_kj (mult_control_idx_injective_qstart bits q_start k j heq)
      rw [style_controlledModAddConst_gate_commute_update_outside_fun_qstart bits q_start N c
            (mult_control_idx_qstart bits q_start j) flagPos
            (mult_control_idx_qstart bits q_start k)
            (m.testBit k) _ h_out h_ne_flag h_ne_ctrl_j]
      rw [cuccaro_target_val_update_outside_workspace bits q_start
            (mult_control_idx_qstart bits q_start k) (m.testBit k) _ h_out]
      exact ih

/-- **R7d^xxix-L-3.15c HEADLINE: q_start-parametric one-step modular-
multiplier target decode.**

After applying `modmult_step_gate_qstart bits q_start N a j flagPos`
to `mult_input_F_qstart bits q_start m acc`, the decoded target
register equals `if m.testBit j then (acc + (a * 2^j) % N) % N else acc`.

Port of `modmult_step_target_decode` (line 820).  Uses the L-3.14′
...
theorem modmult_step_target_decode_qstart
  ...
  -- Bridge mult_input_F_qstart to install over F_j (L-3.15b).
  ...
  have h_ctrl_out :
    ...
    mult_control_idx_outside_modadd_workspace_form_qstart bits q_start j
  have h_flag_out : flagPos < q_start ∨ q_start + 2 * bits + 1 ≤ flagPos :=
    Or.inl h_flag_lt_qstart
  have h_ctrl_ne_flag : mult_control_idx_qstart bits q_start j ≠ flagPos :=
    mult_control_idx_ne_flag_qstart bits q_start j flagPos h_flag_lt_qstart
  have h_ctrl_lt_dim : mult_control_idx_qstart bits q_start j < dim :=
    mult_control_idx_lt_dim_qstart bits q_start j dim hj h_dim_covers_mult
  ...
  have ⟨_, h_tgt, _⟩ :=
    sqir_style_controlledModAddConst_gate_clean_qstart bits q_start N ((a * 2^j) % N) acc dim
      (mult_control_idx_qstart bits q_start j) flagPos (m.testBit j)
...
theorem cuccaro_read_val_through_install_mult
    ...
    cuccaro_read_val bits 2
      ...
      = cuccaro_read_val bits 2
          (Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
            (mult_control_idx bits j) 1) f) := by
  induction num_bits with
  | zero => rfl
  | succ k ih =>
    ...
    · rw [if_neg h_kj]
      have h_out : mult_control_idx bits k < 2
            ∨ 2 + (2 * bits + 1) ≤ mult_control_idx bits k := by
        right; unfold mult_control_idx; omega
      have h_ne_flag : mult_control_idx bits k ≠ 1 := by
        unfold mult_control_idx; omega
      have h_ne_ctrl_j :
          mult_control_idx bits k ≠ mult_control_idx bits j :=
        fun heq => h_kj (mult_control_idx_injective bits k j heq)
      rw [style_controlledModAddConst_gate_commute_update_outside_fun bits N c
            (mult_control_idx bits j) (mult_control_idx bits k)
            (m.testBit k) _ h_out h_ne_flag h_ne_ctrl_j]
      rw [cuccaro_read_val_update_outside_workspace bits 2
            (mult_control_idx bits k) (m.testBit k) _ h_out]
      exact ih
...
theorem applyNat_modmult_through_install_at_workspace
    ...
    Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
      ...
      = Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
        (mult_control_idx bits j) 1) f q := by
  induction num_bits with
  | zero => rfl
  | succ k ih =>
    unfold install_mult_bits_skip_j
    ...
    · rw [if_neg h_kj]
      ...
      rw [style_controlledModAddConst_gate_commute_update_outside_fun bits N c
            (mult_control_idx bits j) (mult_control_idx bits k)
            (m.testBit k) _ h_out h_ne_flag h_ne_ctrl_j]
      have h_q_ne_ctrl_k : q ≠ mult_control_idx bits k := by
        unfold mult_control_idx; omega
...
theorem applyNat_modmult_through_install_at_j
    ...
    Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
      ...
      = Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
        (mult_control_idx bits j) 1) f (mult_control_idx bits j) := by
  induction num_bits with
  | zero => rfl
  | succ k ih =>
    unfold install_mult_bits_skip_j
...
theorem modmult_step_workspace
    ...
    ∧ Gate.applyNat (modmult_step_gate bits N a j)
          (modmult_input_F bits m acc) (mult_control_idx bits j) = m.testBit j := by
  have hacc_lt : acc < 2 ^ bits := Nat.lt_of_lt_of_le hacc hN
  ...
  have h_ctrl_ne_flag : mult_control_idx bits j ≠ 1 :=
    mult_control_idx_ne_flag bits j
  have h_ctrl_lt : mult_control_idx bits j < sqir_modmult_rev_anc bits :=
...
`modmult_step_workspace`.  Mirrors the L-3.15c structure but on
...
theorem applyNat_modmult_through_install_at_workspace_qstart
    ...
    Gate.applyNat (sqir_style_controlledModAddConst_gate bits q_start N c
      ...
      = Gate.applyNat (sqir_style_controlledModAddConst_gate bits q_start N c
        (mult_control_idx_qstart bits q_start j) flagPos) f q := by
  induction num_bits with
  | zero => rfl
  | succ k ih =>
    unfold install_mult_bits_skip_j_qstart
    ...
    · rw [if_neg h_kj]
      ...
      rw [style_controlledModAddConst_gate_commute_update_outside_fun_qstart bits q_start N c
            ...
            (mult_control_idx_qstart bits q_start k)
            (m.testBit k) _ h_out h_ne_flag h_ne_ctrl_j]
      have h_q_ne_ctrl_k : q ≠ mult_control_idx_qstart bits q_start k := by
...
After applying `modmult_step_gate_qstart` to
`mult_input_F_qstart`:
...
Port of `modmult_step_workspace` (line 1055). -/
theorem modmult_step_workspace_qstart
    ...
    ∧ Gate.applyNat (modmult_step_gate_qstart bits q_start N a j flagPos)
          (mult_input_F_qstart bits q_start m acc) flagPos = false
    ∧ Gate.applyNat (modmult_step_gate_qstart bits q_start N a j flagPos)
          (mult_input_F_qstart bits q_start m acc)
          (mult_control_idx_qstart bits q_start j) = m.testBit j := by
  have hacc_lt : acc < 2 ^ bits := Nat.lt_of_lt_of_le hacc hN
  rw [mult_input_F_eq_install_with_j_qstart bits q_start m acc j hj hacc_lt]
  unfold modmult_step_gate_qstart
  have h_ctrl_out :
...
`modmult_prefix_gate_qstart` and the `bits`-fold
`modmult_const_gate_qstart` (plus zero/succ unfold lemmas), and
ports the trivial input-state flag-position helpers
`mult_input_flag_0_false` / `_flag_1_false` to the q_start
...
`mult_input_F_qstart`, the bit is `false`, provided position 0
is below the Cuccaro workspace start.  Port of
`mult_input_flag_0_false` (line 143). -/
theorem mult_input_flag_0_false_qstart
    (bits q_start m acc : Nat) (h_lt : 0 < q_start) :
    mult_input_F_qstart bits q_start m acc 0 = false := by
...
`mult_input_F_qstart`, the bit is `false`, provided position 1
is below the Cuccaro workspace start.  Port of
`mult_input_flag_1_false` (line 151). -/
theorem mult_input_flag_1_false_qstart
    (bits q_start m acc : Nat) (h_lt : 1 < q_start) :
    mult_input_F_qstart bits q_start m acc 1 = false := by
  unfold mult_input_F_qstart
...
theorem modmult_prefix_gate_qstart_zero_eq_I
    ...
    modmult_prefix_gate_qstart bits q_start N a flagPos 0 = Gate.I := rfl
...
theorem modmult_prefix_gate_qstart_succ_eq
    ...
    modmult_prefix_gate_qstart bits q_start N a flagPos (k + 1)
      = seq (modmult_prefix_gate_qstart bits q_start N a flagPos k)
            (modmult_step_gate_qstart bits q_start N a k flagPos) := rfl
