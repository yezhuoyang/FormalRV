/-
  FormalRV.Arithmetic.ModMult.Internal.ToffoliCount — a proved Toffoli (T-count) bound on the
  SAME Gate IR term that the SQIR port proves computes modular multiplication.

  The verified modular multiplier `modmult_const_gate bits N a` is proved to write
  `(a · m) % N` into the accumulator (`modmult_const_gate_target_decode`).  Here we
...
theorem tcount_sqir_modmult_step_gate_le (bits N a j : Nat) :
    tcount (modmult_step_gate bits N a j) ≤ 56 * bits := by
...
theorem tcount_sqir_modmult_prefix_gate_le (bits N a k : Nat) :
    tcount (modmult_prefix_gate bits N a k) ≤ k * (56 * bits) := by
  induction k with
  | zero => simp [modmult_prefix_gate, tcount]
  | succ m ih =>
      simp only [modmult_prefix_gate, tcount]
      calc tcount (modmult_prefix_gate bits N a m)
              + tcount (modmult_step_gate bits N a m)
            ≤ m * (56 * bits) + 56 * bits :=
...
/-- **T-count UPPER BOUND on the verified modular multiplier.**  The SAME Gate term
    `modmult_const_gate bits N a` that `modmult_const_gate_target_decode` proves
    computes `(a · m) % N` costs at most `56·bits²` T-gates (≤ 8·bits² Toffolis). -/
theorem tcount_sqir_modmult_const_gate_le (bits N a : Nat) :
    tcount (modmult_const_gate bits N a) ≤ 56 * bits ^ 2 := by
  unfold modmult_const_gate
  calc tcount (modmult_prefix_gate bits N a bits)
...
/-- **EXACT T-count of the verified modular multiplier.**  `modmult_const_gate bits N a`
...
/-! ## §8. EXACT count of the ACTUAL VERIFIED ORACLE TERM `modmult_MCP_gate`.
    ...
    (`f_modmult_circuit_verified_bits → modmult_MCP_gate`).  In-place = forward
...
theorem tcount_sqir_swap_acc_mult_aux (bits k : Nat) :
    tcount (modmult_swap_acc_mult_aux bits k) = 0 := by
  induction k with
  | zero => rfl
  | succ m ih => simp [modmult_swap_acc_mult_aux, tcount, ih, tcount_qubit_swap]
...
theorem tcount_sqir_encode_to_mult_adapter (bits : Nat) :
    tcount (encode_to_mult_adapter bits) = 0 := by
  simp [encode_to_mult_adapter, tcount_reverse_register_swap]
...
theorem tcount_sqir_modmult_inplace_candidate_eq
    ...
    tcount (modmult_inplace_candidate bits N a ainv) = 112 * bits ^ 2 := by
  simp only [modmult_inplace_candidate, tcount, tcount_sqir_swap_acc_mult]
...
theorem tcount_sqir_modmult_MCP_gate_eq
    ...
    (ha : ∀ j, j < bits → (a * 2 ^ j) % N ≠ 0)
    (hb : ∀ j, j < bits → ((N - ainv) % N * 2 ^ j) % N ≠ 0) :
    tcount (modmult_MCP_gate bits N a ainv) = 112 * bits ^ 2 := by
  simp only [modmult_MCP_gate, modmult_inplace_shifted, tcount,
