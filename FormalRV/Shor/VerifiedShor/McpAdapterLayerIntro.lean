...
def sqirCuccaroLayout : MultiplierStepLayout where
  ...
  multControlIdx   := mult_control_idx
  targetBitIdx     := fun _ i => modmult_target_idx i
  multInputEncode  := modmult_input_F
...
theorem sqirCuccaro_controlIdx_ne_topCarry (bits j : Nat) :
  ...
  mult_control_idx_ne_top_carry bits j
...
theorem sqirCuccaro_controlIdx_injective
    ...
    j = j' :=
  mult_control_idx_injective bits j j' h
...
theorem sqirCuccaro_input_controlBit
  ...
  mult_input_control_bit bits m acc j hj
...
theorem sqirCuccaro_input_eq_install_with_j
  ...
  mult_input_F_eq_install_with_j bits m acc j hj hacc
...
theorem sqirCuccaro_controlledModAdd_commute_install
    ...
    Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
      ...
      = install_mult_bits_skip_j bits m j num_bits
          (Gate.applyNat (sqir_style_controlledModAddConst_gate bits 2 N c
            (mult_control_idx bits j) 1) f) :=
  style_controlledModAddConst_gate_commute_install bits m j N c num_bits f
...
one-step state equality (`modmult_step_state_eq`).
...
theorem sqirCuccaro_step_flag0_false
  ...
  modmult_step_flag0_false bits N a j m acc hbits hj
...
theorem sqirCuccaro_step_above_layout_false
  ...
  modmult_step_above_layout_false bits N a j m acc q hbits hj hq
...
theorem sqirCuccaro_step_carryIn_restored
  ...
  modmult_step_carry_in_restored bits N a j m acc hbits hN_pos hN hN2 hj hacc
...
theorem sqirCuccaro_step_targetBit_extracted
  ...
  modmult_step_target_bit bits N a j m acc i hbits hN_pos hN hN2 hj hacc hi
...
theorem sqirCuccaro_step_readBit_zero
  ...
  modmult_step_read_bit bits N a j m acc i hbits hN_pos hN hN2 hj hacc hi
...
the existing `modmult_step_target_decode` (`ModMult.lean:501`),
...
The original `modmult_step_target_decode` is NOT changed. -/
theorem sqirCuccaro_step_targetDecode_via_interface
  ...
  exact ControlledModAdd.clean_targetDecode ControlledModAdd.sqirCuccaroImpl
    bits N ((a * 2^j) % N) acc (mult_control_idx bits j) (m.testBit j)
...
SQIR-form target decode used by `modmult_step_target_decode`.
...
theorem sqirCuccaro_step_workspace_via_interface
  ...
  show cuccaro_read_val bits 2
      ...
      ∧ Gate.applyNat
            (sqir_style_controlledModAddConst_gate bits 2 N ((a * 2^j) % N)
              (mult_control_idx bits j) 1)
            (modmult_input_F bits m acc) (mult_control_idx bits j)
          = m.testBit j
...
the SQIR-form workspace conjunction used by `modmult_step_workspace`.
Both terms reduce to the same SQIR-level expression via definitional
unfolding through the layout projections, so this is `rfl`. -/
theorem sqirCuccaro_step_workspace_matches_old
    ...
    ∧ Gate.applyNat (modmult_step_gate bits N a j)
          (modmult_input_F bits m acc) 1 = false
    ∧ Gate.applyNat (modmult_step_gate bits N a j)
          (modmult_input_F bits m acc) (mult_control_idx bits j) = m.testBit j) := rfl

/-! ### Step gate well-typedness via interfaces (Phase R6d)

`sqirCuccaro_step_gate_wellTyped_via_interface` proves the same
well-typedness fact as `modmult_step_gate_wellTyped`
(`ModMult.lean:1329`) but stated through the new layout stack
...
theorem sqirCuccaro_step_gate_wellTyped_via_interface
  ...
  exact ControlledModAdd.clean_wellTyped ControlledModAdd.sqirCuccaroImpl
    bits N ((a * 2^j) % N) 0 (mult_control_idx bits j) false
...
SQIR-form well-typedness used by `modmult_step_gate_wellTyped`.
Both terms reduce to the same SQIR-level expression via definitional
unfolding through the layout projections, so this is `rfl`. -/
theorem sqirCuccaro_step_gate_wellTyped_matches_old
    ...
    Gate.WellTyped
      ...
      = Gate.WellTyped (sqir_modmult_rev_anc bits)
          (modmult_step_gate bits N a j) := rfl
...
The original `modmult_step_preserves_all_control_bits`
(`ModMult.lean:774`) splits on `k = j` vs `k ≠ j`:
- `k = j`: the j-th conjunct of `modmult_step_workspace` —
  ...
  · -- k ≠ j case: gate commutes through install; install at controlIdx_k
    ...
    show Gate.applyNat
        (sqir_style_controlledModAddConst_gate bits 2 N ((a * 2^j) % N)
          (mult_control_idx bits j) 1)
        (modmult_input_F bits m acc)
        (mult_control_idx bits k) = m.testBit k
    rw [sqirCuccaro_input_eq_install_with_j bits m acc j hj hacc_lt]
...
`modmult_step_state_eq` (`ModMult.lean:1156`) directly,
because the original `funext q` proof depends on per-position lemmas
(`modmult_step_above_layout_false`, `_flag0_false`,
...
theorem sqirCuccaro_step_state_eq_via_interface
    ...
    Gate.applyNat
      ...
      = sqirCuccaroLayout.multInputEncode bits m
          (if m.testBit j then (acc + (a * 2^j) % N) % N else acc) :=
  modmult_step_state_eq bits N a j m acc hbits hN_pos hN hN2 hj hacc
...
the per-position decoder facts on `modmult_input_F` give all the
ingredients needed to prove `modmult_step_state_eq` through
interface aliases, NOT through `modmult_step_state_eq` or
...
theorem sqirCuccaro_step_state_eq_real_sqir_form
  ...
  · -- Above layout.
    have h_lhs : Gate.applyNat (modmult_step_gate bits N a j)
  ...
  · push_neg at hq_above
    ...
    · push_neg at hq_in_mult
      ...
      · subst hq_1
        ...
        have h_fl : Gate.applyNat (modmult_step_gate bits N a j)
                      (modmult_input_F bits m acc) 1 = false :=
          h_workspace.2.2.1
        rw [h_fl]
      ...
      · subst hq_top
        ...
        have h_tc : Gate.applyNat (modmult_step_gate bits N a j)
                      (modmult_input_F bits m acc) (2 + 2 * bits) = false :=
          h_workspace.2.1
        rw [h_tc]
      ...
      · -- Target bit: q = 2 + 2*i + 1.
        ...
        rw [hq_eq]
        have h_lhs : Gate.applyNat (modmult_step_gate bits N a j)
                      (modmult_input_F bits m acc) (2 + 2 * ((q - 3) / 2) + 1)
      ...
      · -- Read bit: q = 2 + 2*i + 2.
        ...
        rw [hq_eq]
        have h_lhs : Gate.applyNat (modmult_step_gate bits N a j)
                      (modmult_input_F bits m acc) (2 + 2 * ((q - 4) / 2) + 2)
                    = false :=
...
theorem sqirCuccaro_prefix_state_eq_from_via_interface
  ...
  modmult_prefix_state_eq_from bits N a m acc k hbits hN_pos hN hN2 hacc hk
...
theorem sqirCuccaro_const_gate_state_eq_from_via_interface
  ...
  modmult_const_gate_state_eq_from bits N a m acc hbits hN_pos hN hN2 hacc hm
...
(R6f-real) replacing `modmult_step_state_eq`.
...
Neither calls `modmult_step_state_eq`,
`modmult_prefix_state_eq_from`,
`modmult_const_gate_state_eq_from`, or
`sqir_style_controlledModAddConst_gate_clean`. -/
...
theorem sqirCuccaro_prefix_state_eq_from_real_sqir_form
  ...
  | succ n ih =>
    ...
    rw [sqirCuccaro_step_state_eq_real_sqir_form bits N a n m
          (modmult_acc_spec_from N a m acc n)
