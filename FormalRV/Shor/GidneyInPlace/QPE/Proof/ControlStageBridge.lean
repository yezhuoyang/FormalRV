/-
  FormalRV.Shor.GidneyInPlace.QPE.Proof.ControlStageBridge — the CONTROLLED SHIFTED-ORACLE
  jointIdx LAYOUT BRIDGE (live on the hybrid capstone path).
  ════════════════════════════════════════════════════════════════════════════

  Keeps the oracle family `g`/`f` ABSTRACT and translates `uc_eval (control k (map_qubits (·+m) g))`
  into the `jointIdx (phase ⊗ work)` factorization used by the QPE stage decomposition.  Supplies the
  dimension-arithmetic identities (`workDim_eq`, `cast_jointIdx_eq_combine`) consumed downstream by
  `ControlOracleLift`, `Embedding.Def.InPlaceTwoRegEmbedHmarg`, and `Ideal.Proof.InPlaceE2IdealTrajectory`.
-/
...
namespace FormalRV.Shor.GidneyInPlace.ControlStageBridge
...
end FormalRV.Shor.GidneyInPlace.ControlStageBridge