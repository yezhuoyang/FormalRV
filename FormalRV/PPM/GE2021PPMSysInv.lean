...
import FormalRV.System.CodedLayout   -- for `syscall_acts_on`
...
/-! ## §9. Bookkeeping: explicit syscall-kind counts (derived) -/

def count_request_fresh_ancilla (sched : List SysCall) : Nat :=
  sched.foldl (fun acc sc =>
    match sc.kind with
    | .RequestFreshAncilla _ => acc + 1
    | _                      => acc) 0
