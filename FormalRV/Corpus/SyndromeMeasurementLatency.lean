/-
  FormalRV.Corpus.SyndromeMeasurementLatency — syndrome-extraction overhead made
...
namespace FormalRV.Corpus.SyndromeMeasurementLatency

open FormalRV.Framework
open FormalRV.Framework.Resource
open FormalRV.Corpus.SurfaceSystemCompile
...
/-- **IDLE QUBITS ARE NOT FREE.**  A patch with ZERO Toffolis of its own still
...
theorem idle_patch_still_measures (d nToff : Nat) :
    totalPhysicalSyndromeMeas 1 d nToff
      = syndromeAncillaPerPatch d * totalSyndromeRounds nToff d := by
  unfold totalPhysicalSyndromeMeas; rw [Nat.one_mul]
...
end FormalRV.Corpus.SyndromeMeasurementLatency
